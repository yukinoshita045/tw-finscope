...
5. 點 **Deploy**,等 2-3 分鐘
6. 部署完成後會給一個 `https://tw-finscope-xxx.vercel.app` — **這是 demo 用的網址**

### 3.3 回 Render 設 CORS

回 Render → `tw-finscope-api` → 左側 **Environment** → **Add Environment Variable**:
- Key: `CORS_ORIGIN`
- Value: 把你剛拿到的 Vercel 網址貼進去(例如 `https://tw-finscope-xxx.vercel.app`)
...
填在 EXEC_SUMMARY 和 demo 文件:

- **Frontend (demo URL):** `https://<你的 vercel slug>.vercel.app`
- **Backend API:** `https://tw-finscope-api.onrender.com/docs`(FastAPI 自動產生的 Swagger 文件)
- **GitHub repo:** `https://github.com/<你的帳號>/tw-finscope`
- **GitHub Actions:** `https://github.com/<你的帳號>/tw-finscope/actions`(展示 ETL 自動排程)
